...
const outJsonPath = "C:/Users/Lenovo/Downloads/Telegram Desktop/ai_notes.message-sanitized.json";
const outStatsPath = "C:/Users/Lenovo/Downloads/Telegram Desktop/ai_notes.message-sanitized.stats.json";
const outResultsPath = `${root}/.run/ai_notes-message-sanitized-summary-results.json`;
const port = 8022;
...
function sanitizeMessage(content) {
  if (typeof content !== "string") return { text: "", changed: false };
  if (markerRe.test(content)) return { text: "[explicit message omitted]", changed: true };
...
async function main() {
  ...
  const sanitized = source.map((dialogObj, idx) => {
    ...
    if (Array.isArray(clone.dialog)) {
      clone.dialog = clone.dialog.map((msg) => {
        ...
        const res = sanitizeMessage(msg.content);
        if (res.changed) {
          dialogChanged = true;
    ...
    if (dialogChanged) changedDialogs += 1;
    perDialog.push({
      index: idx,
      date: clone.date,
  ...
  fs.writeFileSync(
    outStatsPath,
    JSON.stringify(
      { source: srcPath, output: outJsonPath, dialogs: sanitized.length, changedDialogs, changedMessages, perDialog },
      null,
      2
    ),
    "utf8"
